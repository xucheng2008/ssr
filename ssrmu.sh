...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/xucheng2008/ssr/master/shadowsocksr-manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xucheng2008/ssr/master/ssrmu -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
